...
import { BRIEF_SECTION_ORDER } from '@/intelligence/brief/BriefTypes'
...
function entityMatchesRule(
  entity: BriefGenerationInput['entitySummaries'][number],
  rule: SectionRule,
): boolean {
...
function buildSection(
  ...
  articles: Article[],
  entities: BriefGenerationInput['entitySummaries'],
): BriefSectionData {
...
export class DeterministicBriefGenerator implements BriefGeneratorProvider {
  generate(input: BriefGenerationInput): BriefGenerationResult {
    const { articles, clusters, entitySummaries } = input
    const entityCount = entitySummaries.reduce((sum, entity) => sum + entity.mentionCount, 0)
    const confirmedClusterCount = clusters.filter(
      (cluster) => cluster.agreement === 'Confirmed',
    ).length
    const conflictingReports = clusters.filter(
    ...
    const sections = SECTION_RULES.map((rule) =>
      buildSection(rule, clusters, articles, entitySummaries),
    )
    ...
    const topTechnology = entitySummaries.find((entity) => entity.entityType === 'Technology')
    const topOrganization = entitySummaries.find((entity) => entity.entityType === 'Organization')
    const topCountry = entitySummaries.find((entity) => entity.entityType === 'Country')
    ...
    const executiveSummary = buildSentences([
      `Today's intelligence brief analyzes ${articles.length} articles across ${clusters.length} fused clusters and ${entitySummaries.length} tracked entity groups`,
      topCluster
    ...
    const payload: IntelligenceBriefPayload = {
      ...
      relatedEntities: entitySummaries.slice(0, 20).map((entity) => ({
